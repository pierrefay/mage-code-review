        ...
        try:
            for ligne in fichier:
                ...
                if (result is not None) and (isInScript is not 1) and (intraduct is None) and (isInComment is not 1):
                    if ((code is not 1) or (
                            (varien is None) and (stdclass is None) and (zend is None) and (exception is None) and (
                            soap is None) and (exception is None) and (simplexml is None) and (arrayiterator is None) and (
                                    stdclassbis is None) and (date is None))):
                        retours = {}
                        retours['path'] = path
                        retours['ligne'] = str(nbrLigne)
                        retours['contents'] = (ligne[0:50].strip("     \n\r"))
